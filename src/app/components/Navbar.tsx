...
import logo from "../asset/4fa04261e3f371c7b25530573bb7d9d20806b31e.png";
      ...
      <header className={`sticky top-0 z-50 bg-white/97 backdrop-blur-lg border-b border-[#5B2C6F]/8 transition-shadow duration-300 ${scrolled ? "shadow-md" : "shadow-[0_1px_3px_rgba(91,44,111,0.04)]"}`}>
        ...
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-[72px] sm:h-[80px] lg:h-[96px]" aria-label="Main navigation">
          ...
          <Link to="/" className="flex-shrink-0" aria-label="Divine Counseling Home">
            <img src={logo} alt="Divine Counseling LLC" className="h-[3.5rem] sm:h-[4.25rem] lg:h-[5.5rem] w-auto" />
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-0.5">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`px-2.5 xl:px-3 py-2 rounded-md text-[12.5px] font-[Inter,sans-serif] tracking-wide transition-all duration-200 whitespace-nowrap ${
